...
def test_help():
    ...
    assert out.startswith("Rip v.3.0")
...
# Seems like there is no more 'product' plugin
...
def test_defender_plugin(software_hive):
    tool = dockertools.tool_with_file(__file__)
    tool.output_dirs = [software_hive.relative_to(os.getcwd())]
    out = tool.run_get_string(["-r", str(software_hive), "-p", "defender"])
    assert "Spynet\\SpynetReporting value = 1" in out
    assert "LastWrite Time 2018-01-03 01:24:08Z" in out
