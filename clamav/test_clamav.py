...
def test_entrypoint():
    ...
    assert out.strip().endswith("files inside. The above options ensure safe processing of this kind of data.")
...
def test_do_run_get_tmp_json(tmp_path):
    ...
    d.mkdir()
    tool = dockertools.tool_with_file(__file__)
    out = tool.run_get_string(args=["--gen-json", "--leave-temps", f"--tempdir=^^{d.relative_to(os.getcwd())}/", f"^{SAMPLE_FILE}"])
    # Get files
