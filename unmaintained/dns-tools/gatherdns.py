"""
Copyright 2017 Holmes Group LLC
...
class GatherDNS:
    ...
    def _perform_query(self, domain, rdtype):
        ...
        """
        try:
            logger.info(f"Querying {rdtype} for {domain}")
            result = self.resolver.query(domain, rdtype=rdtype)
        except dns.resolver.NoAnswer:
            logger.exception(f"{rdtype} : The response did not contain a answer for {domain}")
        except dns.resolver.NXDOMAIN:
            logger.exception(f"{rdtype} : The query name does not exist for {domain}")
        except dns.resolver.Timeout:
            logger.exception(f"{rdtype} : The query could not be found in the specified lifetime for {domain}")
        except dns.resolver.NoNameservers:
            logger.exception(f"{rdtype} : No non-broken nameservers are available to answer the question using nameserver {domain}")
        except:
            raise
        else:
            logger.info(f"{rdtype} : Queried {domain} successfully!")
            return result
        return None
